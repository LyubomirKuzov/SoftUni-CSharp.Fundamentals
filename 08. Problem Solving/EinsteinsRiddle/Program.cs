...
class Program
{
    ...
    private static void GenerateHints()
    {
        ...
        hints[13] = $"the {nationalities[0]} lives next to the {houses[0]} house";
    ...
    private static void Shuffle(Random rand)
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            int randomNat = i + rand.Next(0, nationalities.Length - i);
            Swap(i, randomNat, nationalities);

            int randomCig = i + rand.Next(0, cigarettes.Length - i);
            Swap(i, randomCig, cigarettes);
